...
const getWalletNfts = async function (address: string, chain: string) {
    const url = `https://deep-index.moralis.io/api/v2/${address}/nft?chain=${chain}&format=decimal`;
    const result = await axios.get(url, header);
    return result.data.result;
};

const getErc20Balance = async function (address: string, chain: string) {
    const url = `https://deep-index.moralis.io/api/v2/${address}/erc20?chain=${chain}`;
    const result = await axios.get(url, header);
    return result.data;
};

const getContractNfts = async function (address: string, chain: string) {
    const url = `https://deep-index.moralis.io/api/v2/nft/${address}?chain=${chain}&format=decimal`;
    const result = await axios.get(url, header);
    return result.data.result;
};

const getBalance = async function (address: string, chain: string) {
